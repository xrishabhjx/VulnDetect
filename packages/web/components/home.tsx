...
// Format a date on the client only. `new Date(...).toLocaleDateString()` differs
// between Node (UTC) and the browser (local TZ), which breaks hydration.
// Render an empty placeholder on the first paint, then fill it after mount.
function ClientDate({ iso }: { iso: string }) {
  const [formatted, setFormatted] = useState<string>("");
  useEffect(() => {
    setFormatted(new Date(iso).toLocaleDateString());
  }, [iso]);
  // suppressHydrationWarning so React doesn't complain about the empty -> filled swap.
  return <span suppressHydrationWarning>{formatted}</span>;
}

export function HomeClient({ initialScans }: { initialScans: ScanListItem[] }) {
  const router = useRouter();
  const [repoUrl, setRepoUrl] = useState("");
  const [running, setRunning] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [scans, setScans] = useState<ScanListItem[]>(initialScans);
